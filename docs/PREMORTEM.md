...
| No-telemetry promise regresses silently | The landing page promises no telemetry; a later dependency or runtime API call could violate that without changing the core ritual. | Added `npm run verify:local-only`, which scans desktop runtime source for outbound network/telemetry APIs and desktop runtime dependencies for telemetry packages before the no-telemetry promise is considered guarded. |
