...
| Checkout readiness fails closed on malformed Stripe proof | Core proof state and `npm run verify:readiness` reject empty or non-array `line_items.data`, non-object line item entries, malformed quantity fields, and missing price objects instead of throwing or misclassifying malformed proof |
