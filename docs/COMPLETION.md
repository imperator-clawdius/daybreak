...
| Saved-site manifest identity is exported | `site/app/manifest.ts` emits a production-scoped Daybreak web manifest with dark brand colors, productivity category, and the real Electron-captured app image as the icon; `scripts/site-export-metadata.test.ts` proves the manifest file and homepage link exist in the static export, and `scripts/verify-launch.test.ts` proves malformed live manifests keep launch pending |
