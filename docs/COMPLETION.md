...
| Release freshness covers package inputs | `scripts/verify-release.test.ts` requires root dependency manifests, desktop/core TypeScript configs, and desktop icon assets to be included in `RELEASE_SOURCE_PATHS`, so package, build-config, or icon changes make the installer stale until packaging is rerun |
