...
| Checkout readiness fails closed on malformed Stripe proof | Core proof state and `npm run verify:readiness` reject empty or non-array `line_items.data`, non-object line item entries, malformed quantity fields, missing price objects, and malformed price field types instead of throwing or misclassifying malformed proof |
...
| First-order readiness fails closed on malformed paid-order proof | Core proof state and `npm run verify:readiness` reject malformed paid-order Payment Link IDs, amount fields, or currency fields before comparing price, so malformed Stripe exports cannot pass or be misclassified as an ordinary mismatch |
