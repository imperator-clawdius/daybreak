...
| First-order readiness fails closed on malformed paid-order proof | Core proof state and `npm run verify:readiness` reject malformed paid-order amount or currency fields before comparing price, so malformed Stripe exports cannot be misclassified as an ordinary amount mismatch |
