...
| Checkout readiness fails closed on malformed Stripe proof | Core proof state and `npm run verify:readiness` reject malformed Payment Link object/state fields, empty or non-array `line_items.data`, non-object line item entries, malformed quantity fields, missing price objects, malformed recurring fields, and malformed price field types instead of throwing or misclassifying malformed proof |
