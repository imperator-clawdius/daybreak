...
| Unsigned installer packages | `npm run package -w @daybreak/desktop` -> `desktop/release/Daybreak Setup 0.1.0.exe`; current unsigned SHA-256 is `c76adf00a499dd46049489f410a76a585016b6e635fbd860ffa4361314234dc4`; signing skipped because no cert is configured |
...
| Windows release metadata configured | `npm run verify:release` checks app id, product name, root/desktop/core version consistency, author, NSIS x64 target, and installer mode before a release can be considered ready |
...
| Live site surface remains clean | `npm run verify:launch` reports `surface_clean=true` for the primary live site and `WWW_SITE`, and `scripts/verify-launch.test.ts` proves launch stays pending when buyer-visible pages contain common tracker markers, insecure public URLs, or unexpected external HTTPS hosts; failing route entries include the clean-surface reason such as `unexpected_host:cdn.example.com` |
