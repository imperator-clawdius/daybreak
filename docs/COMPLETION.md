...
| Checkout readiness fails closed on malformed Stripe proof | Core proof state and `npm run verify:readiness` reject non-array `line_items.data` and non-object line item entries instead of throwing during static export or readiness verification |
