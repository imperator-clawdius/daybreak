...
| Landing page uses approved generated brand art without overclaiming | `site/public/daybreak-hero-bg.png` is copied from the approved Daybreak image set and used only as atmospheric hero art; `scripts/product-image-dimensions.test.ts` verifies the generated image is present and that Open Graph/Twitter metadata still uses the real `daybreak-app.png` screenshot |
