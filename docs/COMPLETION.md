...
| Support contact is actionable | Homepage, privacy, and terms pages render `founder@daybreak.rest` as `mailto:founder@daybreak.rest`; `scripts/site-export-metadata.test.ts` proves the static export contains two homepage support links plus one each on privacy and terms |
