...
| First-order readiness fails closed on malformed paid-order proof | Core proof state and `npm run verify:readiness` reject malformed paid-order Payment Link IDs, live-mode/status fields, amount fields, or currency fields before comparing Stripe state or price, so malformed Stripe exports cannot pass or be misclassified as an ordinary mismatch |
