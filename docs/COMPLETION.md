...
| Unsigned installer packages | `npm run package -w @daybreak/desktop` -> `desktop/release/Daybreak Setup 0.1.0.exe`; current unsigned SHA-256 is `305ec6564a8b0b6b6ec9bb6b4956cbcbaf75e13845612e62c0c3b1e1b109d02c`; signing skipped because no cert is configured |
...
| Checkout, installer, and paid-order proof reject sensitive data | Core proof state and `npm run verify:readiness` recursively reject proof artifacts containing keys, key-name variants, auth/session headers, customer data, generic personal-data fields, Stripe metadata, client reference IDs, invoices, subscriptions, card or network metadata, payment identifiers, extra order/session snapshots, request/response logs, or certificate private material before other proof-shape mismatches can hide the leak |
