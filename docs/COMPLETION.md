...
| Checkout readiness fails closed on malformed or incomplete Stripe proof | Core proof state and `npm run verify:readiness` reject malformed Payment Link object/state fields, incomplete line-item pagination, empty or non-array `line_items.data`, non-object line item entries, malformed quantity fields, missing price objects, malformed recurring fields, and malformed price field types instead of throwing or misclassifying malformed proof |
