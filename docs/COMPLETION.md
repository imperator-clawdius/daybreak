...
| Hosted installer readiness verifies signing | `npm run verify:readiness` downloads the configured installer bytes, checks SHA-256, rejects unsigned, untimestamped, or wrong-publisher Authenticode signatures from the fetched file, and requires `proof/installer-download.json` to match the hosted signed installer |
