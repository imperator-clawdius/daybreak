...
| Checkout and installer proof reject sensitive data | Core external-link state and `npm run verify:readiness` recursively reject proof artifacts containing keys, customer data, payment identifiers, request/response logs, or certificate private material |
