...
| Checkout, installer, and paid-order proof reject sensitive data | Core proof state and `npm run verify:readiness` recursively reject proof artifacts containing keys, key-name variants, auth/session headers, customer data, payment identifiers, order/session snapshots, request/response logs, or certificate private material before other proof-shape mismatches can hide the leak |
