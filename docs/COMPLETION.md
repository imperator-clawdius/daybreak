...
| Unsigned installer packages | `npm run package -w @daybreak/desktop` -> `desktop/release/Daybreak Setup 0.1.0.exe`; current unsigned SHA-256 is `900cba1863f554516dc91a78b815384de2dd65d7142f086a4156060cda4ddd90`; signing skipped because no cert is configured |
...
| Installer license is configured | `desktop/package.json` sets `build.nsis.license` to `desktop/assets/installer-license.txt`; `npm run verify:release` keeps the release pending if the license is missing before signing or hosting |
