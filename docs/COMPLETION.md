...
| Desktop no-telemetry promise is guarded | `npm run verify:local-only` scans desktop runtime source for outbound network/telemetry APIs and desktop runtime dependencies for telemetry packages; current output is `LOCAL_ONLY=pass` with only `@daybreak/core` as a runtime dependency |
