...
describe("external launch links", () => {
  ...
  it("rejects malformed checkout Payment Link state proof", () => {
    ...
    for (const payment_link of [
      ...
      { ...baseProof.payment_link, id: "plink_live_123 " },
      { ...baseProof.payment_link, id: "plink_live_123!" },
      { ...baseProof.payment_link, id: "plink_live 123" },
    ]) {
      expect(
        getCheckoutProofState({
          checkoutUrl: "https://buy.stripe.com/live_123",
    ...
    expect(
      getCheckoutProofState({
        ...
        proof: {
          ...baseProof,
          payment_link: { ...baseProof.payment_link, id: "link_live_123" },
        },
      }),
    ).toMatchObject({
      ready: false,
  ...
  it("rejects malformed checkout Payment Link URL proof", () => {
    const baseProof = {
      payment_link: {
        url: "https://buy.stripe.com/live_123",
        active: true,
    ...
    expect(
      getCheckoutProofState({
        ...
        proof: {
          ...baseProof,
          payment_link: { ...baseProof.payment_link, url: 123 },
        },
      }),
    ).toMatchObject({
      ready: false,
  ...
  it("rejects checkout proof with discount or promotion fields", () => {
    const baseProof = {
      payment_link: {
        url: "https://buy.stripe.com/live_123",
        active: true,
    ...
    for (const proof of [
      { ...baseProof, allowPromotionCodes: true },
      { ...baseProof, discounts: [{ coupon: "launch" }] },
      { ...baseProof, coupon: "launch" },
      { ...baseProof, promotionCode: "SAVE10" },
      { ...baseProof, totalDetails: { amount_discount: 500 } },
    ]) {
      expect(
        getCheckoutProofState({
          checkoutUrl: "https://buy.stripe.com/live_123",
  ...
  it("rejects installer proof with discount or promotion fields", () => {
    const url = "https://downloads.example.com/daybreak.exe";
    const sha256 =
      "2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824";
    const baseProof = {
    ...
    for (const proof of [
      ...
      { ...baseProof, coupon: "launch" },
      { ...baseProof, promotionCode: "SAVE10" },
      { ...baseProof, totalDetails: { amountDiscount: 500 } },
    ]) {
      expect(
        getVerifiedInstallerLinkState({
          url,
