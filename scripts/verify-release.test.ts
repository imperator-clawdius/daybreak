    ...
    withTempDir((dir) => {
      ...
      const result = evaluatePackagedSmoke({
        ...
        runnerResult: {
          ...
          signal: null,
          stdout:
            "DAYBREAK_SMOKE=pass renderer_loaded=true ipc_roundtrip=true scenario=morning app_menu_disabled=true devtools_disabled=true web_preferences=strict browser_storage=ephemeral launch_args_guarded=true background_throttling=disabled shortcuts_blocked=true print_save_shortcuts_blocked=true close_quit_shortcuts_blocked=true system_shortcuts_blocked=true single_instance_lock=true window_chrome=locked window_ownership=requested permissions_denied=true certificate_errors_rejected=true ipc_sender_guarded=true redirects_guarded=true frame_navigation_guarded=true drag_drop_guarded=true clipboard_exfiltration_guarded=true context_menu_guarded=true malformed_ipc_payload_guarded=true renderer_crash_recovery=recovered downloads_blocked=true content_protection=requested content_protection_status=disabled power_save_blocker=started close_probe=true swipe_flow=true",
          stderr: "",
        },
      });
